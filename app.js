...
let flashDeck=[],flashCurrent=0,flashAnswerShown=false;
const FLASH_MARKED_KEY="pegaso_flashcard_da_ripassare";
function getMarkedFlashcards(){try{return JSON.parse(localStorage.getItem(FLASH_MARKED_KEY)||"[]")}catch(e){return[]}}
function setMarkedFlashcards(ids){localStorage.setItem(FLASH_MARKED_KEY,JSON.stringify(ids))}
function isFlashMarked(id){return getMarkedFlashcards().includes(id)}
function flashParagraphsForModule(m){const qs=m?moduleQuestions(m):DOMANDE;return [...new Set(qs.map(q=>q.paragraph))]}
function initFlashcards(){const ms=[...new Set(DOMANDE.map(q=>canonicalModule(moduleName(q.paragraph))))];$("flashModule").innerHTML='<option value="">Tutti i moduli</option>'+ms.map(m=>`<option value="${esc(m)}">${esc(m)}</option>`).join("");refreshFlashParagraphs();buildFlashDeck()}
function refreshFlashParagraphs(){const m=$("flashModule").value,ps=flashParagraphsForModule(m);$("flashParagraph").innerHTML='<option value="">Tutti i paragrafi</option>'+ps.map(p=>{let d=p.includes(" - ")?p.split(" - ").slice(1).join(" - ").trim():p.trim();d=d.replace(/^\d+\)\s*/,"");return `<option value="${esc(p)}">${esc(d)}</option>`}).join("")}
function buildFlashDeck(){const m=$("flashModule").value,p=$("flashParagraph").value,fm=$("flashMode").value;flashDeck=shuffle(DOMANDE.filter(q=>(!m||canonicalModule(moduleName(q.paragraph))===canonicalModule(m))&&(!p||q.paragraph===p)&&(fm!=="marked"||isFlashMarked(q.id))));flashCurrent=0;flashAnswerShown=false;renderFlashcard()}
function renderFlashcard(){const total=flashDeck.length;$("flashCount").textContent=`${total} flashcard`;$('flashMarked').textContent=`${getMarkedFlashcards().length} da ripassare`;if(!total){$("flashQuestion").textContent="Non ci sono flashcard in questa selezione.";$("flashAnswerText").textContent="";$("flashAnswer").classList.add("hidden");$("flashProgress").textContent="0 / 0";$("flashPrev").disabled=true;$("flashNext").disabled=true;$("flashShow").disabled=true;$("flashMark").disabled=true;return}const q=flashDeck[flashCurrent];$("flashQuestion").textContent=q.question;$("flashAnswerText").textContent=q.correct;$("flashAnswer").classList.toggle("hidden",!flashAnswerShown);$("flashProgress").textContent=`${flashCurrent+1} / ${total}`;$("flashPrev").disabled=flashCurrent===0;$("flashNext").disabled=flashCurrent===total-1;$("flashShow").disabled=false;$("flashShow").textContent=flashAnswerShown?"NASCONDI RISPOSTA":"MOSTRA RISPOSTA";const marked=isFlashMarked(q.id);$("flashMark").disabled=false;$("flashMark").textContent=marked?"★ RIMUOVI DA RIPASSARE":"☆ DA RIPASSARE";$("flashMark").classList.toggle("marked",marked)}
function openFlashcards(){hide("home");hide("moduleSelect");hide("quiz");hide("reviewPage");hide("resultModal");show("flashcards");initFlashcards();window.scrollTo(0,0)}
function closeFlashcards(){hide("flashcards");show("home");window.scrollTo(0,0)}
function toggleFlashAnswer(){if(flashDeck.length){flashAnswerShown=!flashAnswerShown;renderFlashcard()}}
function nextFlash(){if(flashCurrent<flashDeck.length-1){flashCurrent++;flashAnswerShown=false;renderFlashcard()}}
function prevFlash(){if(flashCurrent>0){flashCurrent--;flashAnswerShown=false;renderFlashcard()}}
function toggleFlashMarked(){if(!flashDeck.length)return;const id=flashDeck[flashCurrent].id;let ids=getMarkedFlashcards();ids=ids.includes(id)?ids.filter(x=>x!==id):[...ids,id];setMarkedFlashcards(ids);renderFlashcard()}
...
function startParagraph(p){mode="paragraph";selectedParagraph=p;selectedModule=canonicalModule(moduleName(p));quiz=shuffle(DOMANDE.filter(q=>q.paragraph===p)).map(makeQuestion);startQuiz(p)}
...
function openModule(m){selectedModule=m;$("moduleArea").classList.remove("hidden");$("moduleTitle").textContent=m;$("moduleTestBtn").textContent=`🎯 TEST DEL MODULO — 30 DOMANDE MISTE`;let ps=[...new Set(moduleQuestions(m).map(q=>q.paragraph))];$("paragraphList").innerHTML="";ps.forEach((p,i)=>{let n=DOMANDE.filter(q=>q.paragraph===p).length;let display=p.includes(" - ")?p.split(" - ").slice(1).join(" - ").trim():p.trim();display=display.replace(/^\d+\)\s*/,"");let b=document.createElement("button");b.className="chapter-btn";b.innerHTML=`<strong>${i+1}) ${esc(display)}</strong><span>${n} domande</span>`;b.onclick=()=>startParagraph(p);$("paragraphList").appendChild(b)});setTimeout(()=>{$("moduleArea").scrollIntoView({behavior:"smooth",block:"start"})},80)}
$("flashMode").onclick=openFlashcards;$("flashBack").onclick=closeFlashcards;$("flashModule").onchange=()=>{refreshFlashParagraphs();buildFlashDeck()};$("flashParagraph").onchange=buildFlashDeck;$("flashMode").onchange=buildFlashDeck;$("flashShow").onclick=toggleFlashAnswer;$("flashNext").onclick=nextFlash;$("flashPrev").onclick=prevFlash;$("flashMark").onclick=toggleFlashMarked;
$("examMode").onclick=startExam;$("chapterMode").onclick=openModules;$("reviewBtn").onclick=reviewQuestions;$("downloadBtn").onclick=downloadResults;$("reviewBack").onclick=()=>{hide("reviewPage");$("resultModal").classList.remove("hidden")};$("moduleTestBtn").onclick=()=>startModule(selectedModule);$("finishBtn").onclick=()=>{if(!$("quiz").classList.contains("hidden"))finish()};$("prevBtn").onclick=()=>{if(current>0){current--;renderNav();renderQuestion()}};$("nextBtn").onclick=()=>{if(current<quiz.length-1){current++;renderNav();renderQuestion()}else finish()};$("retryBtn").onclick=()=>{$("resultModal").classList.add("hidden");mode==="exam"?startExam():mode==="module"?startModule(selectedModule):startParagraph(selectedParagraph)};$("nextParagraphBtn").onclick=()=>{let next=getNextParagraph();if(next){$("resultModal").classList.add("hidden");clearInterval(timerInterval);startParagraph(next)}};$("paragraphsBtn").onclick=()=>{$("resultModal").classList.add("hidden");clearInterval(timerInterval);hide("quiz");hide("reviewPage");show("moduleSelect");openModule(selectedModule);setTimeout(()=>{$("moduleArea").scrollIntoView({behavior:"smooth",block:"start"})},120);};$("homeBtn").onclick=home;document.querySelector("[data-back]").onclick=home;
